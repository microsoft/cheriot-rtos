...
class Ksz8851Ethernet
{
	...
	enum [[clang::flag_enum]] ReceiveQueueCommand : uint16_t{
	  ...
	  StartDmaAccess                      = 1 << 3,
	  AutoDequeueReceiveQFrameEnable      = 1 << 4,
	...
	Ksz8851Ethernet()
	{
		...
		register_write(RegisterOffset::TransmitFrameDataPointer, 0x4000);
		// Enable QMU Transmit flow control / Transmit padding / Transmit CRC,
		// and IP/TCP/UDP checksum generation.
		register_write(RegisterOffset::TransmitControl, 0x00EE);
		// Enable QMU Receive Frame Data Pointer Auto Increment.
		register_write(RegisterOffset::ReceiveFrameDataPointer, 0x4000);
		...
		register_write(RegisterOffset::ReceiveControl1, 0x7CE0);
		...
		register_write(RegisterOffset::ReceiveControl2, 0x009C);
		// Enable QMU Receive Frame Count Threshold / ReceiveQ Auto-Dequeue
		// frame.
		register_write(RegisterOffset::ReceiveQueueCommand,
		               ReceiveQueueCommand::ReceiveFrameCountThresholdEnable |
		                 ReceiveQueueCommand::AutoDequeueReceiveQFrameEnable);
		...
		register_set(RegisterOffset::Port1Control, 1 << 13);
		...
		// Enable QMU Transmit.
		register_set(RegisterOffset::TransmitControl, 1 << 0);
		// Enable QMU Receive.
		register_set(RegisterOffset::ReceiveControl1, 1 << 0);
	}
	...
	std::optional<Frame> receive_frame()
	{
		...
		for (; framesToProcess; framesToProcess--)
		{
			...
			// Reset QMU receive frame pointer to zero and start DMA transfer
			// operation.
			register_write(RegisterOffset::ReceiveFrameDataPointer, 0x4000);
